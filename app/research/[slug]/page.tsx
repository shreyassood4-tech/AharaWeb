...
function renderMarkdown(md: string): string {
  return md
    .trim()
    // H2
    .replace(/^## (.+)$/gm, '<h2 class="font-cormorant font-semibold text-charcoal leading-tight mt-10 mb-4" style="font-size:28px">$1</h2>')
    // H3
    .replace(/^### (.+)$/gm, '<h3 class="font-cormorant font-semibold text-charcoal leading-tight mt-8 mb-3" style="font-size:22px">$1</h3>')
    // Bold
    .replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')
    // Ordered list items (must run before paragraph)
    .replace(/^\d+\. (.+)$/gm, '<li class="ml-5 list-decimal">$1</li>')
    // Wrap consecutive <li> in <ol>
    .replace(/(<li[\s\S]*?<\/li>\n?)+/g, (m) => `<ol class="flex flex-col gap-2 my-4 font-inter" style="color:rgba(26,26,26,0.80);font-size:17px;line-height:1.8">${m}</ol>`)
    ...
    .map((block) => {
      ...
      return `<p class="font-inter" style="color:rgba(26,26,26,0.80);font-size:17px;line-height:1.8">${trimmed}</p>`;
    })
    .join('\n');
}
    ...
    <div className="min-h-screen pt-[72px]" style={{ background: "#FAF7F2" }}>
      <div className="max-w-[680px] mx-auto px-6 py-16">
        ...
        <div className="flex items-center gap-6 mt-10 mb-8">
          ...
          <span className="font-mono text-xs tracking-widest" style={{ color: "rgba(26,26,26,0.35)" }}>
        ...
        {/* Headline */}
        <h1
          className="font-cormorant font-light text-charcoal leading-tight mb-8"
              ...
              {related.map((a) => (
                <Link key={a.slug} href={`/research/${a.slug}`}>
                  <div className="glass-card p-6 group">
                    ...
                    <h3 className="font-cormorant text-charcoal text-xl font-semibold group-hover:text-gold transition-colors">
                      {a.title}
                    </h3>
                    <p className="font-inter text-sm mt-2" style={{ color: "rgba(26,26,26,0.60)" }}>
                      {a.summary}
                    </p>
                  </div>
                </Link>
